#ifndef __LSM1MUX_H__
#define __LSM1MUX_H__
...
#define CTRL_REG3_G			0x12//HPCF gyros
#define CTRL_REG4			0x1E//enable gyros
#define CTRL_REG5_XL			0x1F//ACCEL decimation / X Y Z enable
#define CTRL_REG6_XL			0x20//ODR ACCEL / ACCEL SCALE / ACCEL BW / ACCEL AL
#define CTRL_REG9			0x23
#define REFERENCE_G			0x25
#define STATUS_REG_G		0x27
#define OUT_X_L_G			0x18 //GYRO
...
#define OUT_X_L_A_XL			0x28 //ACCEL
#define OUT_X_H_A_XL			0x29 //ACCEL
#define OUT_Y_L_A_XL			0x2A
#define OUT_Y_H_A_XL			0x2B
#define OUT_Z_L_A_XL			0x2C
#define OUT_Z_H_A_XL			0x2D
...
#define CTRL_REG1_M		0x20//temp comp, odr, 
#define CTRL_REG2_M		0x21//scale config
#define CTRL_REG3_M		0x22//operating mode, i2c enable, spi mode, low-pwr mode
...
public:
	...
	enum gyro_scale {
		...
		G_SCALE_1000DPS_NA,	// 10: NOT AVAILABLE - DO NOT USE!
	...
	enum gyro_odr {							// ODR (10*Hz) --- Cutoff
		G_ODR_PWR_DWN0 = 0x00, //   95         12.5
		G_ODR_PWR_DWN1 = 0x01,
		G_ODR_PWR_DWN2 = 0x02,
		G_ODR_PWR_DWN3 = 0x03,
		G_ODR_0149_BW_NA = 0x04, //5, 6, 7 ARE THE SAME
		G_ODR_0595_BW_16 = 0x08,//9, A, B ARE THE SAME
		G_ODR_1190_BW_14 = 0x0C,
		G_ODR_1190_BW_31 = 0x0D, //E, F ARE THE SAME
		G_ODR_2380_BW_14 = 0x10,
		G_ODR_2380_BW_29 = 0x11,
		G_ODR_2380_BW_63 = 0x12,
		G_ODR_2380_BW_78 = 0x13,
		G_ODR_4760_BW_21 = 0x14,
		G_ODR_4760_BW_28 = 0x15,
		G_ODR_4760_BW_57 = 0x16,
		G_ODR_4760_BW_100 = 0x17,
		G_ODR_9520_BW_33 = 0x18,
		G_ODR_9520_BW_40 = 0x19,
		G_ODR_9520_BW_58 = 0x1A,
		G_ODR_9520_BW_100 = 0x1B
	};
	...
	int16_t gx[MAX_MUX], gy[MAX_MUX], gz[MAX_MUX]; // x, y, and z axis readings of the gyroscope //mod to array
	int16_t ax[MAX_MUX], ay[MAX_MUX], az[MAX_MUX]; // x, y, and z axis readings of the accelerometer //mod to array
	int16_t mx[MAX_MUX], my[MAX_MUX], mz[MAX_MUX]; // x, y, and z axis readings of the magnetometer //mod to array
	int16_t temperature[MAX_MUX]; //mod to array
	float abias[MAX_MUX][3]; //mod to array
	float gbias[MAX_MUX][3]; //mod to array
	...
	// 				If MODE_SPI, this is the chip select pin of the accel/gyro (CSXG)
	//	- mAddr = If MODE_I2C, this is the I2C address of the mag.
	//				If MODE_SPI, this is the cs pin of the mag (CSM)
	LSM9DS1(interface_mode interface, uint8_t gAddr, uint8_t xmAddr);
	...
	uint16_t begin(gyro_scale gScl = G_SCALE_245DPS, accel_scale aScl =
			A_SCALE_2G, mag_scale mScl = M_SCALE_4GS, gyro_odr gODR =
			G_ODR_0149_BW_NA, accel_odr aODR = A_ODR_50,
			mag_odr mODR = M_ODR_40);

	uint16_t beginID(short sensorID, gyro_scale gScl = G_SCALE_245DPS, accel_scale aScl =
			A_SCALE_2G, mag_scale mScl = M_SCALE_4GS, gyro_odr gODR =
			G_ODR_0149_BW_NA, accel_odr aODR = A_ODR_50,
			mag_odr mODR = M_ODR_40);
	...
	void setGyroAccelODR(gyro_odr gRate);
	...
	void configGyroInt(uint8_t int1Cfg, uint16_t int1ThsX = 0,
			uint16_t int1ThsY = 0, uint16_t int1ThsZ = 0, uint8_t duration = 0);

	void calLSM9DS1(short sensorID);

	void readMux(short channel);
...
private:
	...
	float gRes, aRes, mRes; //mod. to array
	...
	// xgReadByte() -- Reads a byte from a specified gyroscope register.
	// Input:
	...
	// xgReadBytes() -- Reads a number of bytes -- beginning at an address
	...
	// xgWriteByte() -- Write a byte to a register in the gyroscope.
	// Input:
	//	- subAddress = Register to be written to.
	//	- data = data to be written to the register.
	void xgWriteByte(uint8_t subAddress, uint8_t data);

	// mReadByte() -- Read a byte from a register in the accel/mag sensor
	// Input:
	//	- subAddress = Register to be read from.
	// Output:
	...
	// mReadBytes() -- Reads a number of bytes -- beginning at an address
	...
	// mWriteByte() -- Write a byte to a register in the accel/mag sensor.
	// Input:
	//	- subAddress = Register to be written to.
	//	- data = data to be written to the register.
